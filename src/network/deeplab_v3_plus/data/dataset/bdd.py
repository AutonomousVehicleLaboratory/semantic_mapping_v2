...
class BDDSegmentation(Dataset):
    """
    Berkeley Deep Drive Semantic Segmentation Dataset
    """

    def __init__(self, root_dir, type, transform=None, ignore_index=255):
        """

        Args:
            root_dir (str): The root directory of the dataset. It should be the path to 'bdd100k'.
            type (str): The type of dataset, e.g. ['train', 'val', 'test']
            transform: Data transformation (augmentation)
            ignore_index (int): Index of the pixel label to be ignored
        """
        ...
        self.image_dir = osp.join(self.root_dir, "seg/images", type)
        self.seglabel_dir = osp.join(self.root_dir, "seg/color_labels_preprocessed", type)
        # Check if these directories exist
        assert osp.exists(self.image_dir), "Cannot find {}".format(self.image_dir)
        assert osp.exists(self.seglabel_dir), "Cannot find {} \nDid you preprocess the data?".format(self.seglabel_dir)

        self.image_list = self.get_filenames(self.image_dir)

    def __len__(self):
        return len(self.image_list)

    def __getitem__(self, index):
        image_index = self.image_list[index]
        image = Image.open(osp.join(self.image_dir, image_index + ".jpg"))
        label_image = Image.open(osp.join(self.seglabel_dir, image_index + "_train_color.png"))
...
if __name__ == '__main__':
    ...
    bdd = BDDSegmentation("/home/qinru/avl/semantic/deeplab/data/bdd100k_seg/bdd100k", "train")
    print(len(bdd))

    # Following script is a simple sanity check for the bdd dataset
    sample = bdd[42]
    image = sample["image"]
    label = sample["label"]

    # Iterate through all the data in the dataset, make sure there is no missing files
    ...
    # Following script is used to search for a specific color in the label
    # Note: you need to change the self.seglabel_dir to "seg/color_labels"
    # We discover that all the label that has train_id = 255 does not exist in the semantic segmentation label
    ...
    #
    #     if np.any((label_array == target_color).all(axis=2)):
    #         label_array[(label_array == target_color).all(axis=2)] = (255, 255, 255)
    #
    #         ax = plt.subplot(1, 2, 1)
    #         ax.imshow(image)
    #         ax = plt.subplot(1, 2, 2)
