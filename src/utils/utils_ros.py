...
try:
    ...
    from tf import TransformBroadcaster, TransformListener, TransformerROS, LookupException, ConnectivityException, \
...
def create_point_cloud(xyz, rgb=None, frame_id='world'):
    """ Create point cloud ROS message

    Params:
        xyz: n by 3 float numbers
        rgb: n by 4 array. If provided, they will be the color of each point.
    return:
        point_cloud_msg
    """
    header = Header()
    ...
    if rgb is None:
        ...
        for i in range(xyz.shape[0]):
            rgba_i = \
                struct.unpack('I', struct.Struct('BBBB').pack(int(rgb[i, 0]), int(rgb[i, 1]), int(rgb[i, 2]), 255))[0]
...
def get_transform_from_pose(pose, tf_ros=None):
    """
    Transform the ROS message "pose" into a SE(3) representation.
    ...
    Returns:
        A transformation T in SE(3) space.
